...
const createLetter = async (req, res) => {
    try {
        ...
        if (!name || !FatherName || !rollNo || !gender || !courseName || !enrollmentDate || !trainingPeriod || !collegeName) {
            return res.status(400).json({ message: "All fields are required" });
        }

        const college = await CollageModel.findOne({ name: collegeName });
        if (!college) {
...
const saveCollageName = async (req, res) => {
    try {
        const { collegeName, location, collegeDetails } = req.body;

        console.log("Received college details:", collegeDetails);
        if (!collegeName || !location) {
            return res.status(400).json({ message: "collegeName and location are required" });
        }
        ...
        res.status(201).json({ message: "College saved successfully", collage });
...
const getCollageNames = async (req, res) => {
    try {
        ...
        if (!collageNames || collageNames.length === 0) {
            return res.status(404).json({ message: "No college names found" });
        }
        res.status(200).json({ collageNames: collageNames.map(college => college.name) });
    } catch (error) {
...
const getAllLetters = async (req, res) => {
    try {
        ...
        const letters = await Letter.find().populate('collegeName').sort({ createdAt: -1 }).skip(skip).limit(limit);
                            ...
                            "must": [
                                {
                                    "phrase": {
                                        "query": queryValue1,
                                        "path": "collegeInfo.name", // Searching only in the collegeName field
                                        "score": { "boost": { "value": 5 } }
                                    }
                                },
                                {
...
const updateLetter = async (req, res) => {
    try {
        const { ReferenceNo, name, FatherName, courseName, collegeName, rollNo, gender, enrollmentDate, trainingPeriod, collegeLocation } = req.body;

        let college = await CollageModel.findOne({ name: collegeName });
        if (!college) {
            college = new CollageModel({ name: collegeName, location: collegeLocation });
            await college.save();
        }
        ...
        const updatedLetter = await Letter.findOneAndUpdate(
            { ReferenceNo },
            { name, FatherName, courseName, rollNo, gender, enrollmentDate, trainingPeriod, collegeName: college._id, collegeLocation },
...
const generateBulkCertificates = async (req, res) => {
    try {
        ...
        const requiredFields = ['name', 'FatherName', 'rollNo', 'gender', 'courseName', 'collegeName', 'enrollmentDate', 'trainingPeriod', 'collegeLocation'];
        ...
        for (const row of data) {
            try {
                ...
                let college = await CollageModel.findOne({ name: row.collegeName });
                if (!college) {
                    college = new CollageModel({ name: row.collegeName, location: row.collegeLocation });
                    await college.save();
                }
